  ...
  <div>
    ...
    <main>
      <section class="page-hero">
        <h1>{{ t('groceries.heroLine1') }}<br><span>{{ t('groceries.heroLine2') }}</span></h1>
        ...
        <img src="/images/toma.png" class="hi g1" alt="Tomato">
        <img src="/images/toma.png" class="hi g4" alt="Groceries">
      </section>

      <section class="category-page-section">
        <div class="category-breadcrumb">
          <router-link to="/" class="breadcrumb-link">{{ t('category.home') }}</router-link>
          <span class="breadcrumb-sep">›</span>
          <span class="breadcrumb-current">{{ t('groceries.heroLine1') }}</span>
        </div>
        <div class="subcategory-intro">
          <h3>{{ t('category.browseBy') }}</h3>
          <p>{{ t('groceries.intro') }}</p>
        </div>
        <div class="subcategory-grid">
          <div class="subcategory-card" v-for="cat in groceriesCats" :key="cat.icon">
...
const groceriesCats = computed(() =>
  translations.groceries.cats.map(c => ({
    icon: c.icon,
    name: c.name[lang.value] ?? c.name.en,
    desc: c.desc[lang.value] ?? c.desc.en,
  }))
)
</script>
