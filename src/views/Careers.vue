<template>
  <div class="careers-page">
    ...
    <section class="hero">
      ...
      <img src="/images/shops.png" class="food-item pepper-right" alt="Shop">
    ...
    <section class="why-join">
      <div class="container">
        <h2 class="section-title">Why Work at JamboApp?</h2>
        <p class="section-subtitle">Build your career with a company that's transforming how people access goods and services</p>
        <div class="perks-grid">
          <div class="perk-card" v-for="(perk, index) in perks" :key="index">
      ...
      <div class="container">
        <div class="culture-layout">
          <div class="culture-image">
            <img src="/images/rest.jpg" alt="JamboApp team collaborating">
          </div>
          <div class="culture-content">
            <h2>Our Culture</h2>
            <p>At JamboApp, we believe in building a workplace where innovation meets purpose. Our team is driven by the mission to connect communities through seamless delivery and commerce solutions.</p>
    ...
    <section class="cta-section">
      <div class="container">
        ...
        <p>Join our team and help shape the future of delivery and commerce in Africa</p>
        <div class="cta-buttons">
          <a href="#open-positions" class="cta-btn cta-primary">
            <i class="fas fa-briefcase"></i>
            View Open Positions
          </a>
          <a href="/submit-cv" class="cta-btn cta-secondary">
...
export default {
  ...
  data() {
    return {
      ...
      selectedJob: null,
      perks: [
        { icon: 'fas fa-laptop-house', title: 'Flexible Work', description: 'Hybrid work model with remote options. We trust our team to deliver excellence from anywhere.' },
        { icon: 'fas fa-graduation-cap', title: 'Learning & Growth', description: 'Continuous learning opportunities, conference budgets, and career development programs.' },
        { icon: 'fas fa-heart', title: 'Health & Wellness', description: 'Comprehensive health insurance, mental health support, and wellness programs for you and your family.' },
        { icon: 'fas fa-coins', title: 'Competitive Pay', description: 'Competitive salary packages with performance bonuses and equity options for key roles.' },
        { icon: 'fas fa-users', title: 'Great Team Culture', description: 'Work with passionate, talented people in a collaborative and inclusive environment.' },
        { icon: 'fas fa-calendar-check', title: 'Generous Time Off', description: 'Ample vacation days, sick leave, parental leave, and personal days to recharge.' },
        { icon: 'fas fa-utensils', title: 'Meals & Snacks', description: 'Free lunches, stocked kitchen, and team dining experiences at partner restaurants.' },
        { icon: 'fas fa-birthday-cake', title: 'Celebrations & Events', description: 'Regular team events, annual retreats, and milestone celebrations throughout the year.' }
      ],
      values: [
        { icon: 'fas fa-lightbulb', title: 'Innovation First', description: 'We encourage bold ideas and creative problem-solving at every level.' },
        { icon: 'fas fa-hand-holding-heart', title: 'Community Centered', description: 'Everything we do is focused on serving our communities and partners.' },
        { icon: 'fas fa-shield-alt', title: 'Integrity Always', description: 'We operate with transparency, honesty, and strong ethical principles.' },
        { icon: 'fas fa-fist-raised', title: 'Ownership Mindset', description: 'We take initiative, own outcomes, and drive results with accountability.' }
      ]
  ...
  computed: {
    filteredJobs() {
      if (this.activeDepartment === null || this.activeDepartment === 0) return this.jobs;
      const dept = this.departments[this.activeDepartment];
      return this.jobs.filter(job => job.department === dept);
    }
  }
}
</script>
...
.why-join { padding: 80px 0; background: #14407b; color: #fff; }
.why-join .section-title { color: #fff; }
.why-join .section-subtitle { color: rgba(255, 255, 255, 0.85); }
.perks-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 25px; margin-top: 40px; }
.perk-card { background: rgba(255, 255, 255, 0.1); padding: 30px 25px; border-radius: 15px; border: 1px solid rgba(255, 255, 255, 0.2); transition: all 0.3s ease; }
.perk-card:hover { background: rgba(255, 255, 255, 0.15); transform: translateY(-5px); }
.perk-icon { font-size: 2rem; color: #fdd20a; margin-bottom: 15px; }
.perk-card h3 { font-size: 1.2rem; margin-bottom: 10px; color: #fff; }
.perk-card p { color: rgba(255, 255, 255, 0.8); line-height: 1.5; font-size: 0.95rem; }
.culture { padding: 80px 0; background: #fff; }
.culture-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; }
.culture-image img { width: 100%; height: 450px; object-fit: cover; border-radius: 20px; box-shadow: 0 20px 50px rgba(0,0,0,0.1); }
.culture-content h2 { font-family: 'Playfair Display', serif; font-size: 2.2rem; color: #14407b; margin-bottom: 20px; }
.culture-content > p { color: #666; line-height: 1.7; margin-bottom: 30px; }
.values-list { display: flex; flex-direction: column; gap: 20px; }
.value-item { display: flex; gap: 15px; align-items: flex-start; }
.value-icon { width: 45px; height: 45px; border-radius: 12px; background: #fdfbf4; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; color: #14407b; flex-shrink: 0; }
.value-text h4 { font-size: 1.05rem; color: #14407b; margin-bottom: 4px; }
.value-text p { color: #666; font-size: 0.9rem; line-height: 1.5; }
...
@media (max-width: 768px) {
  ...
  .culture-layout { grid-template-columns: 1fr; gap: 30px; }
  .culture-image img { height: 300px; }
