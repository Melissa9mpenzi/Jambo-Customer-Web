<template>
  <div class="careers-page">
    ...
    <!-- Hero Section -->
    <section class="hero">
      <h1>{{ t('careers.heroLine1') }}<br><span>{{ t('careers.heroLine2') }}</span></h1>
    ...
    <section class="why-join">
      <div class="container">
        <h2 class="section-title">{{ t('careers.whyJoinTitle') }}</h2>
        <p class="section-subtitle">{{ t('careers.whyJoinSub') }}</p>
        <div class="perks-grid">
          <div class="perk-card" v-for="(perk, index) in translatedPerks" :key="index">
      ...
      <div class="container">
        <div class="culture-layout">
          ...
          <div class="culture-content">
            <h2>{{ t('careers.cultureTitle') }}</h2>
            <p>{{ t('careers.cultureBody') }}</p>
            <div class="values-list">
              <div class="value-item" v-for="(value, index) in translatedValues" :key="index">
...
export default {
  ...
  data() {
    return {
      perks: [
        { icon: 'fas fa-laptop-house',    title: { sw: 'Kazi Inayonyumbulika', en: 'Flexible Work' }, description: { sw: 'Mfumo wa kazi mseto na chaguo za mbali. Tunaamini timu yetu kutoa ubora kutoka popote.', en: 'Hybrid work model with remote options. We trust our team to deliver excellence from anywhere.' } },
        { icon: 'fas fa-graduation-cap',  title: { sw: 'Kujifunza na Ukuaji', en: 'Learning & Growth' }, description: { sw: 'Fursa za kujifunza mfululizo, bajeti za mikutano, na programu za maendeleo ya kazi.', en: 'Continuous learning opportunities, conference budgets, and career development programs.' } },
        { icon: 'fas fa-heart',           title: { sw: 'Afya na Ustawi', en: 'Health & Wellness' }, description: { sw: 'Bima ya afya ya kina, msaada wa afya ya akili, na programu za ustawi kwako na familia yako.', en: 'Comprehensive health insurance, mental health support, and wellness programs for you and your family.' } },
        { icon: 'fas fa-coins',           title: { sw: 'Malipo ya Ushindani', en: 'Competitive Pay' }, description: { sw: 'Vifurushi vya mishahara vya ushindani na bonasi za utendaji na chaguo za hisa kwa nyadhifa muhimu.', en: 'Competitive salary packages with performance bonuses and equity options for key roles.' } },
        { icon: 'fas fa-users',           title: { sw: 'Utamaduni Mzuri wa Timu', en: 'Great Team Culture' }, description: { sw: 'Fanya kazi na watu wenye shauku na kipaji katika mazingira ya ushirikiano na ushirikishwaji.', en: 'Work with passionate, talented people in a collaborative and inclusive environment.' } },
        { icon: 'fas fa-calendar-check',  title: { sw: 'Likizo ya Kutosha', en: 'Generous Time Off' }, description: { sw: 'Siku nyingi za likizo, siku za ugonjwa, likizo ya uzazi, na siku za kibinafsi za kupumzika.', en: 'Ample vacation days, sick leave, parental leave, and personal days to recharge.' } },
        { icon: 'fas fa-utensils',        title: { sw: 'Milo na Vitafunio', en: 'Meals & Snacks' }, description: { sw: 'Chakula cha mchana bure, jikoni iliyostokiwa, na uzoefu wa kula pamoja katika mikahawa ya washirika.', en: 'Free lunches, stocked kitchen, and team dining experiences at partner restaurants.' } },
        { icon: 'fas fa-birthday-cake',   title: { sw: 'Sherehe na Matukio', en: 'Celebrations & Events' }, description: { sw: 'Matukio ya mara kwa mara ya timu, safari za kila mwaka, na maadhimisho ya hatua muhimu mwaka mzima.', en: 'Regular team events, annual retreats, and milestone celebrations throughout the year.' } },
      ],
      values: [
        { icon: 'fas fa-lightbulb',       title: { sw: 'Uvumbuzi Kwanza', en: 'Innovation First' }, description: { sw: 'Tunahimiza mawazo ya ujasiri na utatuzi wa ubunifu wa matatizo katika kila ngazi.', en: 'We encourage bold ideas and creative problem-solving at every level.' } },
        { icon: 'fas fa-hand-holding-heart',title: { sw: 'Zingatia Jamii', en: 'Community Centered' }, description: { sw: 'Kila tunachofanya kinalenga kuhudumia jamii zetu na washirika wetu.', en: 'Everything we do is focused on serving our communities and partners.' } },
        { icon: 'fas fa-shield-alt',      title: { sw: 'Uadilifu Daima', en: 'Integrity Always' }, description: { sw: 'Tunafanya kazi kwa uwazi, uaminifu, na kanuni thabiti za maadili.', en: 'We operate with transparency, honesty, and strong ethical principles.' } },
        { icon: 'fas fa-fist-raised',     title: { sw: 'Nia ya Umiliki', en: 'Ownership Mindset' }, description: { sw: 'Tunachukua hatua, tunamiliki matokeo, na tunasukuma matokeo kwa uwajibikaji.', en: 'We take initiative, own outcomes, and drive results with accountability.' } },
      ],
    }
  },
  computed: {
    translatedPerks() {
      return this.perks.map(p => ({ ...p, title: p.title[this.lang] ?? p.title.en, description: p.description[this.lang] ?? p.description.en }))
    },
    translatedValues() {
      return this.values.map(v => ({ ...v, title: v.title[this.lang] ?? v.title.en, description: v.description[this.lang] ?? v.description.en }))
    },
  },
}
</script>
